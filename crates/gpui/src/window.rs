use crate::{
    hash, point, px, size, transparent_black, Action, AnyDrag, AnyElement, AnyTooltip, AnyView,
...
use util::{measure, post_inc, ResultExt};
...
pub trait ElementContext: BorrowWindow {
        ...
        F: FnOnce(&mut Self) -> R,
    {
        if let Some(style) = style {
            self.window_mut().text_style_stack.push(style);
            let result = f(self);
            self.window_mut().text_style_stack.pop();
            result
        } else {
            f(self)
        }
    ...
    fn use_asset<A: Asset + 'static>(&mut self, source: &A::Source) -> Option<A::Output> {
        ...
        S: 'static,
    {
        ...
        self.with_element_id(element_id, |cx| {
            if id_is_none {
                ...
                cx.window_mut().next_frame.accessed_element_states.push(key.clone());
...
impl Borrow<AppContext> for RequestLayoutContext<'_> {
    fn borrow(&self) -> &AppContext {
        self.app
    }
}

impl BorrowMut<AppContext> for RequestLayoutContext<'_> {
    fn borrow_mut(&mut self) -> &mut AppContext {
        self.app
    }
}

impl Borrow<Window> for RequestLayoutContext<'_> {
    fn borrow(&self) -> &Window {
        self.window
    }
}

impl BorrowMut<Window> for RequestLayoutContext<'_> {
    fn borrow_mut(&mut self) -> &mut Window {
        self.window
    }
}
...
impl BorrowMut<AppContext> for PrepaintContext<'_> {
    fn borrow_mut(&mut self) -> &mut AppContext {
        self.app
    }
}

impl Borrow<Window> for PrepaintContext<'_> {
    fn borrow(&self) -> &Window {
        self.window
    }
}

impl BorrowMut<Window> for PrepaintContext<'_> {
    fn borrow_mut(&mut self) -> &mut Window {
        self.window
    }
}
...
impl<'a> PaintContext<'a> {
    ...
    pub fn on_mouse_event<Event: MouseEvent>(
        ...
        self.window.next_frame.mouse_listeners.push(Some(Box::new(
            move |event: &dyn Any, phase: DispatchPhase, cx: &mut WindowContext| {
    ...
    pub fn on_key_event<Event: KeyEvent>(
        ...
        self.window.next_frame.dispatch_tree.on_key_event(Rc::new(
            move |event: &dyn Any, phase, cx: &mut WindowContext| {
    ...
    pub fn on_modifiers_changed(
        ...
        self.window
            ...
            .on_modifiers_changed(Rc::new(
                move |event: &ModifiersChangedEvent, cx: &mut WindowContext| listener(event, cx),
            ));
    }
    ...
    pub fn paint_surface(
        &mut self,
        bounds: Bounds<Pixels>,
        image_buffer: media::core_video::CVImageBuffer,
    ) {
        let scale_factor = self.scale_factor();
        let bounds = bounds.scale(scale_factor);
        let content_mask = self.content_mask().scale(scale_factor);
...
impl Borrow<AppContext> for PaintContext<'_> {
    fn borrow(&self) -> &AppContext {
        self.app
    }
}

impl BorrowMut<AppContext> for PaintContext<'_> {
    fn borrow_mut(&mut self) -> &mut AppContext {
        self.app
    }
}

impl Borrow<Window> for PaintContext<'_> {
    fn borrow(&self) -> &Window {
        self.window
    }
}

impl BorrowMut<Window> for PaintContext<'_> {
    fn borrow_mut(&mut self) -> &mut Window {
        self.window
    }
}
